...
#
# Usage:
#   ci/phase0/run_bench.sh                          # 1 100 1000 4000 graphs
#   ci/phase0/run_bench.sh --quick --graphs 1 100    # smoke run
#   PHASE0_DOCKER_EXTRA='--cpus=4 --memory=6g' \
#       ci/phase0/run_bench.sh --graphs 1 1000
#
...
if [[ ! -x "${REPO_ROOT}/build/output/lgraph_server" ]]; then
    echo "phase0: build/output/lgraph_server not found; run ci/phase0/build.sh first" >&2
    exit 1
fi
