...
struct Fixture {
    ...
    void RegisterShards(std::initializer_list<ShardId> ids) {
        auto txn = store->CreateWriteTxn(false);
        for (ShardId id : ids) EXPECT_TRUE(mgr->RegisterShard(*txn, MakeShard(id)));
        txn->Commit();
        ms->CommitStaged();
    }
    ...
    PlacementVersion Place(const std::string& name, ShardId shard) {
        ...
        auto txn = store->CreateWriteTxn(false);
        if (!ms->PutGraphPlacement(*txn, name, shard, PlacementState::ACTIVE, &v)) {
...
TEST_F(TestRequestRouter, UnknownAndNotActive) {
    ...
    auto r = f.handler->Handle("nope", 0, "payload", f.now);
    ...
    r = f.handler->Handle("g2", 0, "payload", f.now);
...
TEST_F(TestRequestRouter, LocalHandledAndRemoteForwarded) {
    ...
    auto local = f.handler->Handle("local", 0, "payload", f.now);
    EXPECT_EQ(local.disposition, Disposition::HANDLE_LOCALLY);
    EXPECT_EQ(local.target.shard_id, 0u);

    auto remote = f.handler->Handle("away", 0, "payload", f.now);
...
TEST_F(TestRequestRouter, StaleCallerRejectedBeforeForward) {
    ...
    auto first = f.handler->Handle("g1", v1, "payload", f.now);
    ...
    {
        auto txn = f.store->CreateWriteTxn(false);
        f.ms->PutGraphPlacement(*txn, "g1", 1, PlacementState::ACTIVE, &v2);
        txn->Commit();
        f.ms->CommitStaged();
    }
    EXPECT_GT(v2, v1);
    ...
    auto stale = f.handler->Handle("g1", v1, "payload", f.now);
    ...
    auto fresh = f.handler->Handle("g1", v2, "payload", f.now);
...
TEST_F(TestRequestRouter, ForwardErrorSurfaced) {
    ...
    ClusterRequestHandler h(f.ms.get(), f.mgr.get(), f.router.get(), f.control.get(),
                            &failing, 0);
    auto r = h.Handle("away", 0, "payload", f.now);
...
TEST_F(TestRequestRouter, UnhealthyShardNotRouted) {
    ...
    {
        auto txn = f.store->CreateWriteTxn(false);
        f.mgr->SetShardState(*txn, 1, ShardState::OFFLINE);
        txn->Commit();
        f.ms->CommitStaged();
    }
    auto r = f.handler->Handle("away", 0, "payload", f.now + 40000);
